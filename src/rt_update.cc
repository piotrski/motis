      ...
      input.provider_ = timing_source_candidate{
          ...
          .reference_timestamp_seconds_ =
              candidate.provider_reference_timestamp_seconds_.value_or(
                  now_seconds),
          .reference_timestamp_known_ =
              candidate.provider_reference_timestamp_seconds_.has_value(),
          .confidence_ = 1.0,
...
build_vehicle_prediction_diagnostics(
  ...
  auto rendered_events =
      std::map<rendered_event_key,
               vehicle_prediction_overlay_state::rendered_event const*>{};
  for (auto const& event : overlay_state.rendered_events_) {
    auto const range =
        event.trip_stop_range_.value_or(n::interval<n::stop_idx_t>{});
    rendered_events.emplace(
        rendered_event_key{event.transport_, event.trip_stop_range_.has_value(),
                           range.from_, range.to_,
  ...
  for (auto const& selected : selections) {
    ...
    for (auto const& gps : batch.predictions_) {
      ...
      if (selection.source_ == vehicle_prediction_source::kGps &&
        ...
        auto const range =
            source.trip_stop_range_.value_or(n::interval<n::stop_idx_t>{});
        auto const rendered = rendered_events.find(rendered_event_key{
            batch.transport_, source.trip_stop_range_.has_value(), range.from_,
            range.to_, gps.static_stop_sequence_, gps.event_type_});
        if (rendered != end(rendered_events) &&
            rendered->second->selected_timestamp_seconds_.has_value()) {
          effective.predicted_timestamp_seconds_ =
              *rendered->second->selected_timestamp_seconds_;
  ...
  return vehicle_prediction_diagnostics_store::build(
      ...
      {.max_age_seconds_ = max_age,
       .max_entries_ = kMaxVehiclePredictionDiagnosticEntries});
}
  ...
  for (auto const& provider : extracted.candidates_) {
    ...
    auto const add_event = [&](unsigned const sequence,
      ...
      if (!scheduled.has_value() || !predicted.has_value() ||
          !effective.has_value() || *predicted != *effective) {
        return;
      }
        ...
        [&](scheduled_provider_stop const& stop,
          ...
          if (explicit_prediction != end(result.provider_predictions_)) {
            ...
            if ((*propagated_delay > 0 &&
                 *scheduled > std::numeric_limits<std::int64_t>::max() -
                                  *propagated_delay) ||
                (*propagated_delay < 0 &&
                 *scheduled < std::numeric_limits<std::int64_t>::min() -
                                  *propagated_delay)) {
              return;
            }
            add_event(stop.static_stop_sequence_, event_type, scheduled,
                      *scheduled + *propagated_delay, effective);
          }
        };
...
std::vector<vehicle_prediction_cycle_result> merge_provider_candidates(
  ...
  auto index = std::map<std::pair<n::transport, std::string>, std::size_t>{};
  for (auto& provider : providers) {
    auto const key =
        std::pair{provider.batch_.transport_, provider.trip_id_};
    auto const [existing, inserted] = index.try_emplace(key, merged.size());
    if (inserted) {
      merged.emplace_back(std::move(provider));
      continue;
    }
    merge_provider_predictions(merged[existing->second], std::move(provider));
  }
  return merged;
}
          ...
          try {
            ...
            if (vehicle_history != nullptr) {
              ...
              auto candidate_index =
                  std::map<std::pair<n::transport, std::string>, std::size_t>{};
              for (auto const [idx, candidate] : utl::enumerate(candidates)) {
                candidate_index.try_emplace(
                    std::pair{candidate.batch_.transport_, candidate.trip_id_},
                    idx);
              }
              for (auto& provider : provider_candidates) {
                auto const key =
                    std::pair{provider.batch_.transport_, provider.trip_id_};
                auto const existing = candidate_index.find(key);
                if (existing == end(candidate_index)) {
                  candidate_index.emplace(key, candidates.size());
                  candidates.emplace_back(std::move(provider));
                } else {
                  merge_provider_predictions(candidates[existing->second],
                                             std::move(provider));
                }
              }
              ...
              auto overlay_sizes =
                  std::vector<vehicle_prediction_overlay_size>{};
              overlay_sizes.reserve(cycle_selections.size());
              for (auto const& selected : cycle_selections) {
                auto const& candidate = *selected.candidate_;
                if (!gps_selected_transports.contains(
                        candidate.batch_.transport_)) {
                  continue;
                }
                auto const count =
                    selected.selection_.source_ ==
                            vehicle_prediction_source::kSchedule
                        ? candidate.batch_.predictions_.size()
                        : selected.selection_.predictions_.size();
                overlay_sizes.push_back(
                    {.transport_ = candidate.batch_.transport_,
                     .event_count_ = count});
              }
              auto const admitted = select_complete_vehicle_prediction_overlays(
                  overlay_sizes, kMaxVehiclePredictionDiagnosticEntries);
              auto const admitted_overlay_transports =
                  std::set<n::transport>{begin(admitted), end(admitted)};
              for (auto const& selected : cycle_selections) {
                auto const& candidate = *selected.candidate_;
                if (!admitted_overlay_transports.contains(
                        candidate.batch_.transport_)) {
                  continue;
                }
                auto predictions = selected.selection_.predictions_;
                if (selected.selection_.source_ ==
                    vehicle_prediction_source::kSchedule) {
                  predictions = candidate.batch_.predictions_;
                  for (auto& prediction : predictions) {
                    prediction.predicted_timestamp_seconds_ =
                        prediction.scheduled_timestamp_seconds_;
                    prediction.delay_seconds_ = 0;
                  }
                }
