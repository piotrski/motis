...
std::vector<trip_progress_diagnostic> evaluate_trip_progress_diagnostics(
  ...
  auto evaluated_positions = std::size_t{0U};
  for (auto const& vehicle : positions.all()) {
    auto diagnostic = trip_progress_diagnostic{
        ...
        .mode_ = std::nullopt,
        .status_ = trip_progress_diagnostic_status::kUnresolvedTrip,
        .lateral_error_m_ = std::nullopt};
    if (!feed_enabled(c, control, diagnostic.feed_)) {
      continue;
    }
    ...
    if (evaluated_positions++ >= kMaxVehiclePredictionPositionsPerCycle) {
      break;
    }
    ...
    if (configured_mode(c, control, diagnostic.feed_, *diagnostic.mode_) ==
        config::timetable::vehicle_eta::mode::off) {
      continue;
    }
