...
std::string_view dataset_tag(std::string_view const feed_id) {
  auto const pos = feed_id.rfind(':');
...
bool route_matches(tag_lookup const& tags,
  ...
  auto const src = tags.get_src(dataset_tag(vehicle.feed_id_));
...
std::optional<n::rt::frun> resolve_run(
  ...
  auto td = to_trip_descriptor(vehicle);
  if (!td.has_trip_id() && !(td.has_route_id() && td.has_direction_id() &&
                             td.has_start_date() && td.has_start_time())) {
    return std::nullopt;
  }
  if (!td.has_trip_id() && src < tt.route_ids_.size()) {
    auto const& routes = tt.route_ids_[src];
    if (auto const route = find_route_id(routes, td.route_id());
        route.has_value()) {
      td.set_route_id(std::string{routes.ids_.get(*route)});
    }
  }
...
bool matches_service_day(tag_lookup const& tags,
  ...
  auto const target_id = tags.id(tt, target[0], n::event_type::kDep);
  return target_id.starts_with(*vehicle.trip_.start_date_) &&
         target_id.size() > vehicle.trip_.start_date_->size() &&
...
bool can_match_target_trip(tag_lookup const& tags,
  ...
  auto const target_ids =
      tt.trip_ids_[target[0].get_trip_idx(n::event_type::kDep)];
  ...
  return route_matches(tags, tt, vehicle, target_route);
}
  ...
  for (auto const& vehicle : store.all()) {
    ...
    if (!matches_target) {
      if (auto fr = resolve_static_trip_run_by_id(tags, tt, vehicle);
          fr.has_value() && matches_service_day(tags, tt, target, vehicle)) {
