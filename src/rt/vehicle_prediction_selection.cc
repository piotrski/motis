...
std::optional<scheduled_provider_stop> find_scheduled_stop(
    resolved_provider_trip const& resolved, std::string_view const stop_id) {
  auto const it = std::ranges::find(resolved.stops_, stop_id,
                                    &scheduled_provider_stop::stop_id_);
  return it == end(resolved.stops_) ? std::nullopt : std::optional{*it};
}
...
provider_timing_extraction extract_provider_timing(
  ...
  for (auto const& entity : message.entity()) {
    ...
    auto resolved = std::optional<resolved_provider_trip>{};
    try {
      resolved = resolve(update.trip());
    } catch (...) {
    ...
    auto candidate = provider_timing_candidate{
        ...
        .resolved_stops_ = resolved->stops_};
    for (auto const& source_stop : update.stop_time_update()) {
      if (source_stop.has_schedule_relationship() &&
          source_stop.schedule_relationship() ==
              transit_realtime::TripUpdate_StopTimeUpdate::NO_DATA) {
        continue;
      }
      auto const scheduled =
          source_stop.has_stop_sequence()
              ? find_scheduled_stop(*resolved, source_stop.stop_sequence())
              : (source_stop.has_stop_id()
                     ? find_scheduled_stop(*resolved, source_stop.stop_id())
                     : std::nullopt);
      if (!source_stop.has_stop_sequence() && !scheduled.has_value()) {
        continue;
      }
      auto const sequence = source_stop.has_stop_sequence()
                                ? source_stop.stop_sequence()
                                : scheduled->static_stop_sequence_;
