...
void vehicle_position_store::update_feed(
  ...
  std::erase_if(missed_full_snapshot_once_, [&](auto const& key) {
    return key.first == feed_id &&
           (std::ranges::find(deleted_entity_ids, key.second) !=
  ...
  std::erase_if(positions_, [&](vehicle_position const& existing) {
    ...
    auto const deleted =
        std::ranges::find(deleted_entity_ids, existing.entity_id_) !=
        end(deleted_entity_ids);
    auto const replaced =
        std::ranges::find(positions, existing.entity_id_,
                          &vehicle_position::entity_id_) != end(positions);
    auto const superseded_vehicle =
        existing.vehicle_.id_.has_value() &&
        std::ranges::any_of(positions, [&](vehicle_position const& incoming) {
          return incoming.vehicle_.id_ == existing.vehicle_.id_;
        });
    return deleted || replaced || superseded_vehicle;
  });
  for (auto& pos : positions) {
    pos.feed_id_ = feed_id;
  }
