...
Vehicle ETA runtime controls default to
`<data-directory>/runtime/vehicle-eta`. Set
`MOTIS_VEHICLE_ETA_RUNTIME_DIR` to keep controls and calibration outside a
replaceable timetable data directory, such as `/data/runtime/vehicle-eta` in a
...
timetable:                          # if not set, no timetable will be loaded
  ...
  vehicle_eta:                      # optional vehicle-derived prediction policy; omitted defaults to off
    ...
    selection:
      ...
      early_departure_tolerance_seconds: 0
      minute_rounding_deadband_seconds: 10
    modes:                          # optional transit-mode overrides
      BUS: shadow
    feeds:                          # optional dataset-tag overrides; feed wins over transit mode, which wins over global
      nl:
        modes: [BUS]                # omit to apply this feed override to every transit mode
        mode: off
