...
TEST(provider_timing_extraction, resolves_timing_identified_only_by_stop_id) {
  auto message = transit_realtime::FeedMessage{};
  message.mutable_header()->set_gtfs_realtime_version("2.0");
  auto* const update = message.add_entity()->mutable_trip_update();
  update->mutable_trip()->set_trip_id("trip");
  auto* const timed = update->add_stop_time_update();
  timed->set_stop_id("stop-b");
  ...
  ASSERT_EQ(extracted.candidates_.size(), 1U);
  ASSERT_EQ(extracted.candidates_.front().stops_.size(), 1U);
  EXPECT_EQ(extracted.candidates_.front().stops_.front().static_stop_sequence_,
            20U);
  EXPECT_EQ(
...
TEST(provider_timing_extraction,
     isolates_unresolvable_updates_from_valid_candidates) {
  auto message = transit_realtime::FeedMessage{};
  message.mutable_header()->set_gtfs_realtime_version("2.0");
  for (auto const trip_id : {"bad", "good"}) {
    auto* const update = message.add_entity()->mutable_trip_update();
    update->mutable_trip()->set_trip_id(trip_id);
    auto* const stop = update->add_stop_time_update();
    stop->set_stop_sequence(20U);
    stop->mutable_departure()->set_delay(60);
  }

  auto const extracted = extract_provider_timing(
      message, [](transit_realtime::TripDescriptor const& trip) {
        if (trip.trip_id() == "bad") {
          throw std::runtime_error{"unresolvable provider trip"};
        }
        return std::optional{resolved_provider_trip{
            .transport_ = transport(),
            .stops_ = {{.static_stop_sequence_ = 20U,
  ...
  EXPECT_EQ(extracted.unresolved_trip_updates_, 1U);
  ASSERT_EQ(extracted.candidates_.size(), 1U);
  ASSERT_EQ(extracted.candidates_.front().stops_.size(), 1U);
  EXPECT_EQ(
      extracted.candidates_.front().stops_.front().departure_timestamp_seconds_,
